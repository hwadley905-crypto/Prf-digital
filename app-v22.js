...
// v22 exit-to-home workflow.
function returnToReadyScreen(){
  const gate=$('startupGate'),ready=$('readyPanel'),loading=$('loadingPanel'),audio=$('newIncidentAudio');
  $('exitCaseModal').classList.remove('open');
  if(audio){audio.pause();audio.currentTime=0;}
  if(gate){gate.classList.remove('is-closed','effects-off');}
  if(ready){ready.hidden=false;const p=ready.querySelector('p');if(p)p.textContent='Select Yes to continue.';}
  if(loading)loading.hidden=true;
  window.scrollTo({top:0,left:0,behavior:'auto'});
}
$('exitCase').onclick=()=>$('exitCaseModal').classList.add('open');
$('exitSave').onclick=()=>{
  const data=collect();
  const defaultName=data.fields.patient_name||data.caseId;
  const name=prompt('Case name',defaultName);
  if(name===null)return;
  data.name=name.trim()||defaultName;
  localStorage.setItem('prf_saved_'+Date.now(),JSON.stringify(data));
  localStorage.setItem('prf_current',JSON.stringify(data));
  returnToReadyScreen();
};
$('exitWithoutSave').onclick=()=>{
  localStorage.removeItem('prf_current');
  returnToReadyScreen();
};
