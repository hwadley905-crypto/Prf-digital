...
// v18 stable NEWS2 and left/right pupil controller.
(() => {
  const install = () => {
    const byId = id => document.getElementById(id);
    const obsTable = byId('obsTable');
    const newsModal = byId('newsModal');
    const newsTotal = byId('newsTotal');
    const pupilModal = byId('pupilPairModal');
    const leftSelect = byId('fixedPupilLeft');
    const rightSelect = byId('fixedPupilRight');
    if (!obsTable || !newsModal || !newsTotal) return;

    // Force modal visibility independently of any older cached/open-class behaviour.
    ...
    let fixedNewsButton = null;
    let fixedPupilButton = null;

    if (leftSelect && rightSelect && !leftSelect.options.length) {
      for (let n = 0; n <= 10; n += 1) {
        leftSelect.add(new Option(`${n} mm`, String(n)));
        rightSelect.add(new Option(`${n} mm`, String(n)));
      }
    }
    const pupilSide = (text, side) => {
      const m = String(text || '').match(new RegExp('(10|[0-9])\\s*' + side, 'i'));
      return m ? m[1] : '4';
    };

    // Capture phase runs before the legacy onclick handler and prevents it corrupting the target.
    document.addEventListener('click', event => {
      const newsButton = event.target.closest('#obsTable .news-button');
      if (newsButton) {
        event.preventDefault();
        event.stopImmediatePropagation();
        fixedNewsButton = newsButton;
        window.__fixedNewsSelections = {};
        document.querySelectorAll('#newsTable td.selected').forEach(cell => cell.classList.remove('selected'));
        newsTotal.textContent = '0';
        showModal(newsModal);
        return;
      }
      const pupilButton = event.target.closest('#obsTable .pupil-button');
      if (pupilButton && pupilModal) {
        event.preventDefault();
        event.stopImmediatePropagation();
        fixedPupilButton = pupilButton;
        leftSelect.value = pupilButton.dataset.left || pupilSide(pupilButton.textContent, 'L');
        rightSelect.value = pupilButton.dataset.right || pupilSide(pupilButton.textContent, 'R');
        showModal(pupilModal);
      }
    }, true);

    // Replace NEWS2 cell selection with a self-contained score store.
    document.querySelectorAll('#newsTable tbody tr').forEach((row, rowIndex) => {
      [...row.cells].slice(1).forEach(cell => {
        const scoreClass = [...cell.classList].find(c => /^news-score-[0-3]$/.test(c));
        if (!scoreClass) return;
        cell.addEventListener('click', event => {
          event.preventDefault();
          event.stopImmediatePropagation();
          const score = Number(scoreClass.slice(-1));
          window.__fixedNewsSelections = window.__fixedNewsSelections || {};
          window.__fixedNewsSelections[rowIndex] = score;
          [...row.cells].forEach(c => c.classList.remove('selected'));
          cell.classList.add('selected');
          newsTotal.textContent = String(Object.values(window.__fixedNewsSelections).reduce((a, b) => a + Number(b), 0));
        }, true);
      });
    });

    const done = byId('newsDone');
    if (done) done.addEventListener('click', event => {
      if (!fixedNewsButton) return;
      event.preventDefault();
      event.stopImmediatePropagation();
      const total = Number(newsTotal.textContent || 0);
      const red = Object.values(window.__fixedNewsSelections || {}).includes(3);
      fixedNewsButton.textContent = String(total);
      fixedNewsButton.dataset.redScore = String(red);
      fixedNewsButton.classList.remove('news-low','news-low-medium','news-medium','news-high');
      fixedNewsButton.classList.add(total >= 7 ? 'news-high' : total >= 5 ? 'news-medium' : red ? 'news-low-medium' : 'news-low');
      hideModal(newsModal);
      if (typeof saveDraft === 'function') saveDraft();
    }, true);

    document.querySelectorAll('[data-close="newsModal"]').forEach(button => button.addEventListener('click', event => {
      event.preventDefault(); event.stopImmediatePropagation(); hideModal(newsModal);
    }, true));

    const pupilDone = byId('fixedPupilDone');
    if (pupilDone) pupilDone.addEventListener('click', event => {
      if (!fixedPupilButton) return;
      event.preventDefault(); event.stopImmediatePropagation();
      const left = leftSelect.value, right = rightSelect.value;
      fixedPupilButton.dataset.left = left;
      fixedPupilButton.dataset.right = right;
      fixedPupilButton.dataset.values = JSON.stringify([`${left}L`, `${right}R`]);
      fixedPupilButton.textContent = `${left}L ${right}R`;
      fixedPupilButton.title = `Left pupil ${left} mm; right pupil ${right} mm`;
      hideModal(pupilModal);
      if (typeof saveDraft === 'function') saveDraft();
    }, true);

    document.querySelectorAll('[data-close="pupilPairModal"]').forEach(button => button.addEventListener('click', event => {
      event.preventDefault(); event.stopImmediatePropagation(); hideModal(pupilModal);
    }, true));

    // Label every current and future row clearly.
    const labelRows = () => document.querySelectorAll('#obsTable .pupil-button').forEach(button => {
      const left = button.dataset.left || pupilSide(button.textContent, 'L');
      const right = button.dataset.right || pupilSide(button.textContent, 'R');
      button.dataset.left = left; button.dataset.right = right;
      button.textContent = `${left}L ${right}R`;
      button.title = `Left pupil ${left} mm; right pupil ${right} mm`;
    });
    labelRows();
    new MutationObserver(labelRows).observe(obsTable.tBodies[0], {childList: true});
  };
