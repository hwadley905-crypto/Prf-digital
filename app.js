...
$('showAlsGuidelines').onclick=()=>{const modal=$('alsGuidelinesModal'),frame=$('alsGuidelinesFrame'),pdf='assets/WMAS%20ALS%20Checklist.pdf';if(frame&&frame.getAttribute('src')!==pdf)frame.setAttribute('src',pdf);modal.classList.add('open');};
$('showUniAlsGuide').onclick=()=>$('uniAlsGuideModal').classList.add('open');
...
function playPrfAudio(id,{restart=true}={}){const a=document.getElementById(id);if(!a)return Promise.resolve(false);if(restart){a.pause();a.currentTime=0;}const p=a.play();return p&&p.then?p.then(()=>true).catch(()=>false):Promise.resolve(true);}
function stopPrfAudio(id){const a=document.getElementById(id);if(a){a.pause();a.currentTime=0;}}
...
function returnToReadyScreenV2404(){
 playPrfAudio('sepuraNotificationAudio');stopPrfAudio('sepuraPtpCallAudio');
 hideExitCaseModal();
...
// v24.2.0 mixed MP3/M4A audio controls.
window.addEventListener('DOMContentLoaded',()=>{
 ...
 const all=()=>[...document.querySelectorAll('audio')],msg=t=>status.textContent=t;
 const apply=()=>{const v=Number(slider.value)/100;all().forEach(a=>{a.volume=v;a.muted=mute.checked;});value.textContent=`${slider.value}%`;localStorage.setItem('prf_audio_volume',slider.value);localStorage.setItem('prf_audio_muted',mute.checked);};
 slider.value=localStorage.getItem('prf_audio_volume')||'100';mute.checked=localStorage.getItem('prf_audio_muted')==='true';apply();open.onclick=()=>modal.classList.add('open');slider.oninput=apply;mute.onchange=apply;
 preload.onclick=()=>{all().forEach(a=>a.load());msg('Audio preload requested. The browser will use the first supported MP3 or M4A source it can load.');};
 test.onclick=async()=>msg(await playPrfAudio('sepuraNotificationAudio')?'Notification audio played.':'Notification audio could not be played. Check that either the MP3 or M4A file exists.');
 stop.onclick=()=>{all().forEach(a=>{a.pause();a.currentTime=0;});msg('All application audio stopped.');};
});
