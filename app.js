...
function createObservationButtonCell(row,className,value,onClick){
...
window.addObs=function(v={}){
  ...
  const avpuButton=createObservationButtonCell(row,'avpu-button',v.avpu||'-',button=>{
    openPicker('AVPU',['A - Alert','C - New confusion','V - Voice','P - Pain','U - Unresponsive'],button,false);
  });

  const pupilButton=createObservationButtonCell(row,'pupil-button',v.pupils||'-',button=>{
    openPicker('Select up to two pupil values',['0 mm','1 mm','2 mm','3 mm','4 mm','5 mm','6 mm','7 mm','8 mm','9 mm','10 mm'],button,true);
  });
  pupilButton.dataset.values=JSON.stringify(v.pupilValues||[]);

  const gcsButton=createObservationButtonCell(row,'gcs-button',v.gcs||'-',button=>{
    activeGcs=button;
    $('gcsModal').classList.add('open');
    refreshGcs();
  });

  const newsButton=createObservationButtonCell(row,'news-button',v.news2??'-',button=>{
    activeNews=button;
    newsSelections={};
  ...
  newsButton.title='Open NEWS2 calculator';
  newsButton.setAttribute('aria-label','Open NEWS2 calculator');
