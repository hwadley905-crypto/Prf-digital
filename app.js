...
// v19 stable NEWS2/pupil controller and NEWS2 >=5 sepsis-tool display.
(() => {
  const install = () => {
    const $id = id => document.getElementById(id);
    const obsTable=$id('obsTable'),newsModal=$id('newsModal'),newsTotal=$id('newsTotal');
    const pupilModal=$id('pupilPairModal'),leftSelect=$id('fixedPupilLeft'),rightSelect=$id('fixedPupilRight');
    const sepsisPanel=$id('sepsisToolPanel');
    if(!obsTable||!newsModal||!newsTotal)return;
    const showModal=modal=>{modal.classList.add('open');modal.style.setProperty('display','flex','important');modal.style.setProperty('visibility','visible','important');modal.style.setProperty('opacity','1','important');};
    const hideModal=modal=>{modal.classList.remove('open');modal.style.removeProperty('display');modal.style.removeProperty('visibility');modal.style.removeProperty('opacity');};
    let selectedNewsButton=null,selectedPupilButton=null;
    window.__newsSelectionsV19={};
    if(leftSelect&&rightSelect&&!leftSelect.options.length){for(let n=0;n<=10;n++){leftSelect.add(new Option(`${n} mm`,String(n)));rightSelect.add(new Option(`${n} mm`,String(n)));}}
    const pupilValue=(text,side)=>{const m=String(text||'').match(new RegExp('(10|[0-9])\\s*'+side,'i'));return m?m[1]:'4';};
    const updateSepsisPanel=()=>{
      const high=[...obsTable.querySelectorAll('.news-button')].some(button=>Number(button.textContent)>=5);
      if(sepsisPanel)sepsisPanel.hidden=!high;
    };
    document.addEventListener('click',event=>{
      const newsButton=event.target.closest('#obsTable .news-button');
      if(newsButton){event.preventDefault();event.stopImmediatePropagation();selectedNewsButton=newsButton;window.__newsSelectionsV19={};document.querySelectorAll('#newsTable td.selected').forEach(c=>c.classList.remove('selected'));newsTotal.textContent='0';showModal(newsModal);return;}
      const pupilButton=event.target.closest('#obsTable .pupil-button');
      if(pupilButton&&pupilModal){event.preventDefault();event.stopImmediatePropagation();selectedPupilButton=pupilButton;leftSelect.value=pupilButton.dataset.left||pupilValue(pupilButton.textContent,'L');rightSelect.value=pupilButton.dataset.right||pupilValue(pupilButton.textContent,'R');showModal(pupilModal);}
    },true);
    document.querySelectorAll('#newsTable tbody tr').forEach((row,rowIndex)=>{
      [...row.cells].slice(1).forEach(cell=>{
        const cls=[...cell.classList].find(c=>/^news-score-[0-3]$/.test(c));if(!cls)return;
        cell.addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();window.__newsSelectionsV19[rowIndex]=Number(cls.slice(-1));[...row.cells].forEach(c=>c.classList.remove('selected'));cell.classList.add('selected');newsTotal.textContent=String(Object.values(window.__newsSelectionsV19).reduce((a,b)=>a+Number(b),0));},true);
      });
    });
    $id('newsDone')?.addEventListener('click',event=>{
      if(!selectedNewsButton)return;event.preventDefault();event.stopImmediatePropagation();const total=Number(newsTotal.textContent||0),red=Object.values(window.__newsSelectionsV19).includes(3);selectedNewsButton.textContent=String(total);selectedNewsButton.dataset.redScore=String(red);selectedNewsButton.classList.remove('news-low','news-low-medium','news-medium','news-high');selectedNewsButton.classList.add(total>=7?'news-high':total>=5?'news-medium':red?'news-low-medium':'news-low');hideModal(newsModal);updateSepsisPanel();if(typeof saveDraft==='function')saveDraft();
    },true);
    document.querySelectorAll('[data-close="newsModal"]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();hideModal(newsModal);},true));
    $id('fixedPupilDone')?.addEventListener('click',event=>{if(!selectedPupilButton)return;event.preventDefault();event.stopImmediatePropagation();const l=leftSelect.value,r=rightSelect.value;selectedPupilButton.dataset.left=l;selectedPupilButton.dataset.right=r;selectedPupilButton.dataset.values=JSON.stringify([`${l}L`,`${r}R`]);selectedPupilButton.textContent=`${l}L ${r}R`;selectedPupilButton.title=`Left pupil ${l} mm; right pupil ${r} mm`;hideModal(pupilModal);if(typeof saveDraft==='function')saveDraft();},true);
    document.querySelectorAll('[data-close="pupilPairModal"]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();hideModal(pupilModal);},true));
    const normalisePupils=()=>document.querySelectorAll('#obsTable .pupil-button').forEach(b=>{const l=b.dataset.left||pupilValue(b.textContent,'L'),r=b.dataset.right||pupilValue(b.textContent,'R');b.dataset.left=l;b.dataset.right=r;b.textContent=`${l}L ${r}R`;b.title=`Left pupil ${l} mm; right pupil ${r} mm`;});
    normalisePupils();updateSepsisPanel();
    new MutationObserver(()=>{normalisePupils();updateSepsisPanel();}).observe(obsTable.tBodies[0],{childList:true,subtree:true,characterData:true});
    $id('closeSepsisTool')?.addEventListener('click',()=>{if(sepsisPanel)sepsisPanel.hidden=true;});
  };
