...
// v24.0.3 robust exit workflow and diagnostic.
window.__prfExitWorkflowReady=false;
window.__prfOpenExitDialog=function(){
  const modal=document.getElementById('exitCaseModal');
...
window.addEventListener('load',()=>{
  const exitButton=document.getElementById('exitCase');
  ...
  window.__prfExitWorkflowReady=Boolean(exitButton&&exitModal&&exitSave&&exitWithoutSave);
  if(exitButton){
    exitButton.addEventListener('click',event=>{
      event.preventDefault();event.stopImmediatePropagation();window.__prfOpenExitDialog();
    },true);
  }
  if(window.runPrfSystemCheck){
    const originalCheck=window.runPrfSystemCheck;
    window.runPrfSystemCheck=()=>{
      const baseResult=originalCheck();
      ...
      if(holder){
        ...
        row.innerHTML=`<span>Function: Exit workflow</span><span class="${window.__prfExitWorkflowReady?'health-ok':'health-fail'}">${window.__prfExitWorkflowReady?'Ready':'Missing'}</span>`;
        holder.appendChild(row);
      }
      return baseResult&&window.__prfExitWorkflowReady;
    };
  }
});
