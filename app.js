...
function parsePupilValue(text,side){const match=String(text||'').match(new RegExp('(10|[0-9])\\s*'+side,'i'));return match?match[1]:null;}
for(let size=0;size<=10;size++){
  const leftOption=document.createElement('option');leftOption.value=String(size);leftOption.textContent=`${size} mm`;$('pupilLeft').appendChild(leftOption);
  const rightOption=document.createElement('option');rightOption.value=String(size);rightOption.textContent=`${size} mm`;$('pupilRight').appendChild(rightOption);
}
function openPupilModal(button){
  activePupilButton=button;
  $('pupilLeft').value=button.dataset.left||parsePupilValue(button.textContent,'L')||'4';
  $('pupilRight').value=button.dataset.right||parsePupilValue(button.textContent,'R')||'4';
  $('pupilModal').classList.add('open');
}
$('pupilDone').addEventListener('click',()=>{
  if(!activePupilButton)return;
  const left=$('pupilLeft').value,right=$('pupilRight').value;
  activePupilButton.dataset.left=left;activePupilButton.dataset.right=right;
  ...
  $('pupilModal').classList.remove('open');saveDraft();
});
function observationButtonCell(row,action,className,text){
  const td=row.insertCell(),button=document.createElement('button');
  button.type='button';button.dataset.obsAction=action;button.className=`table-button ${className}`;button.textContent=text??'-';td.appendChild(button);return button;
}
window.addObs=function(v={}){
  ...
  const avpu=observationButtonCell(row,'avpu','avpu-button',v.avpu||'-');avpu.dataset.values=JSON.stringify(v.avpu&&v.avpu!=='-'?[v.avpu]:[]);
  const pupils=observationButtonCell(row,'pupils','pupil-button',v.pupils||'4L 4R');pupils.dataset.left=String(v.pupilLeft??parsePupilValue(v.pupils,'L')??4);pupils.dataset.right=String(v.pupilRight??parsePupilValue(v.pupils,'R')??4);pupils.dataset.values=JSON.stringify([`${pupils.dataset.left}L`,`${pupils.dataset.right}R`]);pupils.textContent=`${pupils.dataset.left}L ${pupils.dataset.right}R`;
  observationButtonCell(row,'gcs','gcs-button',v.gcs||'-');
  const news=observationButtonCell(row,'news','news-button',v.news2??'-');news.title='Open NEWS2 calculator';news.setAttribute('aria-label','Open NEWS2 calculator');if(v.news2!==undefined&&v.news2!=='-')paintNews(news,Number(v.news2),Boolean(v.newsRed));
  deleteCell(row);
};
$('obsTable').addEventListener('click',event=>{
  const button=event.target.closest('button[data-obs-action]');if(!button)return;
  event.preventDefault();event.stopPropagation();
  const action=button.dataset.obsAction;
  if(action==='avpu'){openPicker('AVPU',['A - Alert','C - New confusion','V - Voice','P - Pain','U - Unresponsive'],button,false);return;}
  if(action==='pupils'){openPupilModal(button);return;}
  if(action==='gcs'){activeGcs=button;$('gcsModal').classList.add('open');refreshGcs();return;}
  if(action==='news'){
    activeNews=button;newsSelections={};document.querySelectorAll('#newsTable td.selected').forEach(cell=>cell.classList.remove('selected'));
    $('newsTotal').textContent='0';$('newsModal').classList.add('open');return;
  }
});
...
$('newsDone').onclick=()=>{if(!activeNews)return;const total=Number($('newsTotal').textContent),red=Object.values(newsSelections).includes(3);activeNews.textContent=String(total);activeNews.dataset.redScore=String(red);paintNews(activeNews,total,red);$('newsModal').classList.remove('open');saveDraft();};
