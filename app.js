...
$('showAlsGuidelines').onclick=()=>{const modal=$('alsGuidelinesModal'),frame=$('alsGuidelinesFrame'),localPdf='assets/als-guidelines.pdf';if(frame&&frame.getAttribute('src')!==localPdf)frame.setAttribute('src',localPdf);modal.classList.add('open');};
$('showUniAlsGuide').onclick=()=>{const modal=$('uniAlsGuideModal'),frame=$('uniAlsGuideFrame'),localPdf='assets/university-als-guide.pdf';if(frame&&frame.getAttribute('src')!==localPdf)frame.setAttribute('src',localPdf);modal.classList.add('open');};
...
function returnToReadyScreenV2404(){
 ...
 stopPrfAudio('sepuraPtpCallAudio');
 hideExitCaseModal();
...
// v24.1.8 application audio controls.
window.addEventListener('DOMContentLoaded',()=>{
 const open=document.getElementById('audioControls'),modal=document.getElementById('audioControlsModal'),slider=document.getElementById('audioVolume'),value=document.getElementById('audioVolumeValue'),mute=document.getElementById('audioMute'),preload=document.getElementById('preloadAudio'),test=document.getElementById('testNotificationAudio'),stop=document.getElementById('stopAllAudio'),status=document.getElementById('audioControlStatus');
 if(!open||!modal||!slider||!mute)return;const all=()=>[...document.querySelectorAll('audio')];const message=t=>{if(status)status.textContent=t;};
 const apply=()=>{const volume=Number(slider.value)/100;all().forEach(a=>{a.volume=volume;a.muted=mute.checked;});value.textContent=`${slider.value}%`;try{localStorage.setItem('prf_audio_volume',slider.value);localStorage.setItem('prf_audio_muted',String(mute.checked));}catch{}};
 try{const v=localStorage.getItem('prf_audio_volume'),m=localStorage.getItem('prf_audio_muted');if(v!==null)slider.value=v;if(m!==null)mute.checked=m==='true';}catch{}apply();
 open.onclick=()=>{apply();modal.classList.add('open');};slider.oninput=()=>{apply();message(`Volume set to ${slider.value}%.`);};mute.onchange=()=>{apply();message(mute.checked?'Application audio muted.':'Application audio unmuted.');};
 preload.onclick=()=>{const items=all();items.forEach(a=>a.load());message(`Preload requested for ${items.length} audio files.`);};
 test.onclick=async()=>{apply();message(await playPrfAudio('sepuraNotificationAudio')?'Notification audio played.':'Notification audio could not be played. Check the audio file.');};
