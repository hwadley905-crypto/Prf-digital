...
// v24.0.2 robust exit workflow diagnostics
window.__exitWorkflowReady=false;
window.addEventListener('load',()=>{
 const exitBtn=document.getElementById('exitCase');
 const exitModal=document.getElementById('exitCaseModal');
 const saveBtn=document.getElementById('exitSave');
 const noSaveBtn=document.getElementById('exitWithoutSave');
 if(exitBtn && exitModal){
   exitBtn.onclick=()=>{exitModal.classList.add('open');};
   window.__exitWorkflowReady=true;
 }
 if(saveBtn){saveBtn.dataset.diagnostic='ready';}
 if(noSaveBtn){noSaveBtn.dataset.diagnostic='ready';}
 if(window.runPrfSystemCheck){
   const old=window.runPrfSystemCheck;
   window.runPrfSystemCheck=()=>{
     const result=old();
     const holder=document.getElementById('healthResults');
     if(holder){
       holder.insertAdjacentHTML('beforeend',`<div class="health-item"><span>Function: Exit workflow</span><span class="${window.__exitWorkflowReady?'health-ok':'health-fail'}">${window.__exitWorkflowReady?'Ready':'Missing'}</span></div>`);
     }
     return result;
   };
 }
