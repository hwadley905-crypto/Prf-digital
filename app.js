...
const pages=['incident','patient','primary','secondary','history','observations','consent','trauma','interventions','drugs','als','outcome','signoff'];
const labels={incident:'Incident',patient:'Patient',primary:'Primary',secondary:'Secondary',history:'History',observations:'Observ.',consent:'COI',trauma:'Trauma',interventions:'Intervent.',drugs:'Drugs',als:'ALS',outcome:'Outcome',signoff:'Sign off'};
...
buildGrid('incidentGrid',schemas.incident);buildGrid('patientGrid',schemas.patient);buildGrid('secondaryGrid',schemas.secondary);buildGrid('historyGrid',schemas.history);buildGrid('outcomeGrid',schemas.outcome);buildGrid('timingsGrid',schemas.timings);
...
$('ecgUpload').onchange=()=>previewFiles($('ecgUpload'),$('ecgPreview'));$('injuryUpload').onchange=()=>previewFiles($('injuryUpload'),$('injuryPreview'));$('historyCameraUpload').onchange=()=>previewFiles($('historyCameraUpload'),$('historyPreview'));$('historyImageUpload').onchange=()=>previewFiles($('historyImageUpload'),$('historyPreview'));$('historyFileUpload').onchange=()=>previewFiles($('historyFileUpload'),$('historyPreview'));$('showPpci').onclick=()=>$('ppciModal').classList.add('open');
$('timings').onclick=()=>$('timingsModal').classList.add('open');$('timingsDone').onclick=()=>{$('timingsModal').classList.remove('open');saveDraft();};$('printPdf').onclick=()=>{growAll();document.body.classList.add('prf-printing');setTimeout(()=>window.print(),50);};window.addEventListener('afterprint',()=>document.body.classList.remove('prf-printing'));
...
$('showAlsGuidelines').onclick=()=>openLocalPdf('assets/WMAS ALS Checklist.pdf');
$('showUniAlsGuide').onclick=()=>openLocalPdf('assets/university-als-guide.pdf');
...
function collectGpHistory(){const tbody=document.querySelector('#gpHistoryTable tbody');if(!tbody)return [];return [...tbody.rows].map(row=>({date:row.querySelector('.gp-history-input')?.value||'',context:row.querySelector('[aria-label="Context"]')?.value||'',information:row.querySelector('[aria-label="Information"]')?.value||'',clinician:row.querySelector('[aria-label="Clinician"]')?.value||''}));}
function collect(){const data={version:7,caseId:$('caseId').textContent,savedAt:new Date().toISOString(),priority:localStorage.getItem('prf_priority'),fields:{},obs:serializeRows('obs'),interventions:serializeRows('interventions'),drugs:serializeRows('drugs'),als:serializeRows('als'),clinicians:collectClinicians(),gpHistory:collectGpHistory()};document.querySelectorAll('[data-key]').forEach(x=>data.fields[x.dataset.key]=x.type==='checkbox'?x.checked:x.value);return data;}
function saveDraft(){localStorage.setItem('prf_current',JSON.stringify(collect()));}
function populate(data){document.querySelectorAll('[data-key]').forEach(x=>{const v=data.fields?.[x.dataset.key];if(v!==undefined)x.type==='checkbox'?x.checked=!!v:x.value=v;});['obs','interventions','drugs','als'].forEach(n=>$(n+'Table').tBodies[0].innerHTML='');(data.obs||[]).forEach(addObs);(data.interventions||[]).forEach(v=>addTableRow('interventions',v));(data.drugs||[]).forEach(v=>addTableRow('drugs',v));(data.als||[]).forEach(v=>addTableRow('als',v));window.__pendingGpHistory=Array.isArray(data.gpHistory)?data.gpHistory:[];window.dispatchEvent(new CustomEvent('gp-history-loaded'));restoreClinicians(data.clinicians||[]);growAll();updatePrimary();updateAlert();}
...
const PRF_AUDIO_SOURCES={
 newIncidentAudio:['assets/New Incident.mp3','assets/New Incident.m4a'],
 sepuraNotificationAudio:['assets/Sepura Notification.mp3','assets/Sepura Notification.m4a'],
 sepuraPtpCallAudio:['assets/Sepura PTP Call.mp3','assets/Sepura PTP Call.m4a']
};
function configurePrfAudio(id){const audio=document.getElementById(id),sources=PRF_AUDIO_SOURCES[id]||[];if(!audio||!sources.length)return;let index=0;const load=()=>{audio.src=sources[index];audio.load();};audio.addEventListener('error',()=>{if(index<sources.length-1){index++;load();}},{capture:true});load();}
function playPrfAudio(id,{restart=true}={}){const audio=document.getElementById(id);if(!audio)return Promise.resolve(false);if(restart){audio.pause();audio.currentTime=0;}const result=audio.play();return result&&typeof result.then==='function'?result.then(()=>true).catch(()=>false):Promise.resolve(true);}
function stopPrfAudio(id){const audio=document.getElementById(id);if(audio){audio.pause();audio.currentTime=0;}}
function openLocalPdf(path){const url=new URL(path,window.location.href).href;window.open(url,'_blank','noopener,noreferrer');}
['newIncidentAudio','sepuraNotificationAudio','sepuraPtpCallAudio'].forEach(configurePrfAudio);
...
let exitSequenceActive=false;
function beginExitSequence(){
 if(exitSequenceActive)return;exitSequenceActive=true;
 hideExitCaseModal();
 const overlay=document.getElementById('exitLoadingOverlay');if(overlay){overlay.hidden=false;overlay.style.display='flex';}
 playPrfAudio('sepuraNotificationAudio');
 setTimeout(()=>{stopPrfAudio('sepuraNotificationAudio');if(overlay){overlay.hidden=true;overlay.style.display='none';}exitSequenceActive=false;returnToReadyScreenV2404();},5000);
}
...
function exitWithoutSavingV2404(){
 localStorage.removeItem('prf_current');window.__prfExitLastAction='Exit without saving completed';beginExitSequence();
}
window.addEventListener('load',()=>{
...
(() => {
  const onReady=()=>{
    ...
    const eligible=textarea=>{
      ...
      return Boolean(page && page.id!=='incident' && page.id!=='patient' && !textarea.classList.contains('no-auto-expand') && !textarea.closest('.modal,.v23-overlay,.text-editor-overlay,.gp-history-page'));
    };
...
// v24.1.2 GP History saved in JSON, print-excluded, consent-gated and password-locked for editing.
(() => {
  const init=()=>{
    const open=document.getElementById('openGpHistory'),consent=document.getElementById('gpConsentModal');
    const yes=document.getElementById('gpConsentYes'),no=document.getElementById('gpConsentNo');
    const loading=document.getElementById('gpLoadingOverlay'),page=document.getElementById('gpHistoryPage');
    const close=document.getElementById('closeGpHistory'),add=document.getElementById('addGpHistoryRow');
    const unlock=document.getElementById('unlockGpHistory'),lock=document.getElementById('lockGpHistory');
    const status=document.getElementById('gpEditStatus'),tbody=document.querySelector('#gpHistoryTable tbody'),message=document.getElementById('gpHistoryMessage');
    if(!open||!consent||!yes||!no||!loading||!page||!tbody)return;
    const PASSWORD='NeeNaw999';let editing=false;
    const showMessage=text=>{message.textContent=text;clearTimeout(showMessage.timer);showMessage.timer=setTimeout(()=>message.textContent='',3500);};
    const checkPassword=()=>{const entered=window.prompt('Enter password');if(entered===null)return false;if(entered!==PASSWORD){showMessage('Incorrect password');return false;}return true;};
    const applyLock=()=>{tbody.querySelectorAll('input,textarea').forEach(el=>el.disabled=!editing);tbody.querySelectorAll('.gp-delete-row').forEach(el=>el.disabled=!editing);add.disabled=!editing;unlock.hidden=editing;lock.hidden=!editing;status.textContent=editing?'Editing enabled':'Read only';};
    const addRow=(data={})=>{
      const row=tbody.insertRow();
      const date=row.insertCell(),dateInput=document.createElement('input');dateInput.type='date';dateInput.className='gp-history-input';dateInput.dataset.transientGpHistory='true';dateInput.value=data.date||'';date.appendChild(dateInput);
      [['Context',data.context],['Information',data.information],['Clinician',data.clinician]].forEach(([label,value])=>{const cell=row.insertCell(),area=document.createElement('textarea');area.className='gp-history-entry no-auto-expand';area.dataset.transientGpHistory='true';area.rows=3;area.setAttribute('aria-label',label);area.value=value||'';cell.appendChild(area);});
      const action=row.insertCell(),del=document.createElement('button');del.type='button';del.className='gp-delete-row';del.textContent='Delete row';del.onclick=()=>{if(!checkPassword())return;row.remove();saveDraft();};action.appendChild(del);applyLock();
    };
    const renderPending=()=>{tbody.innerHTML='';const rows=Array.isArray(window.__pendingGpHistory)?window.__pendingGpHistory:[];rows.forEach(addRow);if(!rows.length)addRow();editing=false;applyLock();};
    window.addEventListener('gp-history-loaded',renderPending);
    open.onclick=()=>consent.classList.add('open');no.onclick=()=>consent.classList.remove('open');
    yes.onclick=()=>{consent.classList.remove('open');loading.hidden=false;loading.style.display='flex';playPrfAudio('sepuraPtpCallAudio');setTimeout(()=>{stopPrfAudio('sepuraPtpCallAudio');loading.hidden=true;loading.style.display='none';renderPending();page.hidden=false;page.style.display='block';document.body.classList.add('gp-history-open');},7000);};
    close.onclick=()=>{editing=false;applyLock();page.hidden=true;page.style.display='none';document.body.classList.remove('gp-history-open');saveDraft();};
    unlock.onclick=()=>{if(!checkPassword())return;editing=true;applyLock();};lock.onclick=()=>{editing=false;applyLock();saveDraft();};
    add.onclick=()=>{if(!editing){showMessage('Enable editing first');return;}if(!checkPassword())return;addRow();saveDraft();};
    renderPending();
  };
...
window.addEventListener('DOMContentLoaded',()=>{
 const open=$('audioControls'),modal=$('audioControlsModal'),slider=$('audioVolume'),value=$('audioVolumeValue'),mute=$('audioMute'),preload=$('preloadAudio'),test=$('testNotificationAudio'),stop=$('stopAllAudio'),status=$('audioControlStatus');if(!open||!modal)return;
 const all=()=>[...document.querySelectorAll('audio')],message=t=>status.textContent=t;
 const apply=()=>{const v=Number(slider.value)/100;all().forEach(a=>{a.volume=v;a.muted=mute.checked;});value.textContent=`${slider.value}%`;try{localStorage.setItem('prf_audio_volume',slider.value);localStorage.setItem('prf_audio_muted',String(mute.checked));}catch{}};
 try{slider.value=localStorage.getItem('prf_audio_volume')||'100';mute.checked=localStorage.getItem('prf_audio_muted')==='true';}catch{}apply();
 open.onclick=()=>modal.classList.add('open');slider.oninput=apply;mute.onchange=apply;
 preload.onclick=()=>{all().forEach(a=>a.load());message('Audio preload requested. Each sound will try MP3, then M4A.');};
 test.onclick=async()=>{apply();const ok=await playPrfAudio('sepuraNotificationAudio');message(ok?'Notification audio played.':'Notification audio could not be played. Check the exact filename and assets folder.');};
 stop.onclick=()=>{all().forEach(a=>{a.pause();a.currentTime=0;});message('All application audio stopped.');};
});
