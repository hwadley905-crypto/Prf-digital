...
window.addObs=function(v={}){
  ...
  const newsButton=addControl(row,'news-button',v.news2??'-',button=>{activeNews=button;newsSelections={};document.querySelectorAll('#newsTable td.selected').forEach(x=>x.classList.remove('selected'));$('newsTotal').textContent='0';$('newsModal').classList.add('open');});
  ...
  deleteCell(row);updateSepsisPanel();
};
